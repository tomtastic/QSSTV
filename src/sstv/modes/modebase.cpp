...
const QString stateStr[modeBase::MBTXGAPROBOT + 1] = {"ERROR",  "SETUPLINE", "PIXELS", "SYNC",  "ENDOFLINE", "EOIMAGE",
...
/**
  ...
  This function initializes all buffers and mode parameters. The localClock will be set to rxClk if rxClk is not zero in
  receive mode, else the rxClock of the configuration will be used. The localClock is always equal to the txClock from
  the configuration while in transmit mode;
...
/**
  ...
  This function is the default behaviour. it calls combineColors(). This function must be reimplemented in the derived
...
/**
  ...
  Get the YUV information from the image and setup the colour arrays. The displayCounter is incremented by 1 or 2,
  depending on the mode (e.g. 1 if Robot24 and 2 if Robot36).
