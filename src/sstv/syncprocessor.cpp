...
void syncProcessor::process()
{
  ...
  if(!detectNarrow)
  {
    ...
    extractSync(inputVolumePtr,RXSTRIPE);
#ifdef DISABLENARROW
  }
#endif
...
void syncProcessor::extractSync(DSPFLOAT* buffer, size_t length) {
    int lastSync = 0;

    // Optimize main loop
    for (size_t i = 0; i < length; ++i) {
        DSPFLOAT volume = buffer[i];
        DSPFLOAT syncVolume = syncVolumePtr[i];

        switch (syncState) {
        case SYNCOFF:
        case SYNCVALID:
            // Skip if below minimum volume
            if (volume < sensitivityArray[sensitivity].minVolume) continue;

            // Check sync activation condition
            if (syncVolume > sensitivityArray[sensitivity].onRatio * volume) {
                syncArray[syncArrayIndex].start = sampleCounter + i;
                syncArray[syncArrayIndex].startVolume = syncVolume;
                switchSyncState(SYNCACTIVE, sampleCounter + i);
            }
            break;

        case SYNCACTIVE:
            // Check for volume drop below minimum
            if (volume < sensitivityArray[sensitivity].minVolume) {
                switchSyncState(SYNCOFF, sampleCounter + i);
                continue;
            }

            // Check sync deactivation condition
            if (syncVolume < sensitivityArray[sensitivity].offRatio * volume) {
                syncArray[syncArrayIndex].end = sampleCounter + i;

                // Validate and switch state accordingly
                if (validateSync()) {
...
#ifndef QT_NO_DEBUG
        syncStateBuffer[i] = static_cast<unsigned char>(syncState) * STATESCALER;
#endif
    }

    // Handle post-loop sync processing
    if (syncProcesState == INSYNC && enableSyncDetection) {
        if (activeChainPtr == nullptr) {
            ...
            DSPFLOAT timeThreshold = lastSync + sensitivityArray[sensitivity].maxTempOutOfSyncLines * samplesPerLine;
            if ((sampleCounter + length - length / 7) > timeThreshold) {
                if (sensitivity != (NUMBEROFSENSITIVITIES - 1)) {
                    tempOutOfSync = true;  // Allow temporary out of sync for non-DX modes
                }
            }
            missingLines = static_cast<uint>(
                round(((sampleCounter + length - length / 7) - (timeThreshold)) / samplesPerLine + 1));
            calcSyncQuality();
        }
    }

    // Handle VIS-based mode switching
...
bool syncProcessor::validateSync()
{
  ...
  {
    if ((syncArrayIndex > 2) && (syncArray[syncArrayIndex].end - syncArray[syncArrayIndex - 1].start) >= MINRETRACEWIDTH && (syncArray[syncArrayIndex].start - syncArray[syncArrayIndex - 1].end) <= MINRETRACEWIDTH / 5)
    {
...
bool syncProcessor::addToChain(esstvMode mode, uint fromIdx)
{
  ...
  if ((syncArray[syncArrayIndex].diffStartEnd() < syncWidth * MINSYNCWIDTHRATIO) || (syncArray[syncArrayIndex].diffStartEnd() > syncWidth * MAXSYNCWIDTHRATIO) || (syncArray[fromIdx].diffStartEnd() < syncWidth * MINSYNCWIDTHRATIO))
  {
