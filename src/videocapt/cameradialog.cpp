      ...
      while (v4l2_ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frm) >= 0) {
        ...
        } else  // we have a stepwise resolution
        {
          ...
          for (j = 0; j < NUMRES; j++) {
            ...
            if ((resx <= frm.stepwise.max_width) && (resy <= frm.stepwise.max_height) &&
                (resx >= frm.stepwise.min_width) && (resy >= frm.stepwise.min_height))
...
bool cameraDialog::restartCapturing(bool first) {
  ...
  if (!videoCapturePtr->init(
          cameraList.at(ui->devicesComboBox->currentIndex()).formats.at(ui->formatsComboBox->currentIndex()).format,
          cameraList.at(ui->devicesComboBox->currentIndex())
              .formats.at(ui->formatsComboBox->currentIndex())
              .cameraSizes.at(ui->sizeComboBox->currentIndex())
              .width,
          cameraList.at(ui->devicesComboBox->currentIndex())
              .formats.at(ui->formatsComboBox->currentIndex())
              .cameraSizes.at(ui->sizeComboBox->currentIndex())
