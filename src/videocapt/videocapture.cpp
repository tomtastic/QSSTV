...
bool videoCapture::convert(unsigned char* src) {
  ...
  if (needsConversion) {
    result = v4lconvert_convert(convertData, &srcFmt, &dstFmt, src, static_cast<int>(srcFmt.fmt.pix.sizeimage),
...
bool videoCapture::captureStart() {
  ...
  if (!mmapped) {
    ...
    for (i = 0; i < numBuffers; i++) {
      ...
      mem[i] =
          static_cast<uchar*>(v4l2_mmap(nullptr, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, dev, buf.m.offset));
      if (mem[i] == MAP_FAILED) {
