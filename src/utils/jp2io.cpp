...
QImage jp2IO::decode(QString fileName)
{
  ...
  if (!parameters.nb_tile_to_decode) {
    ...
    if (!opj_set_decode_area(l_codec, jp2Image, static_cast<OPJ_INT32>(parameters.DA_x0),
                             static_cast<OPJ_INT32>(parameters.DA_y0), static_cast<OPJ_INT32>(parameters.DA_x1),
  ...
  if (jp2Image->icc_profile_buf) {
    ...
    jp2Image->icc_profile_buf = nullptr;
    jp2Image->icc_profile_len = 0;
  }
...
opj_image_t* jp2IO::convert_gray_to_rgb(opj_image_t* original)
{
  ...
  l_new_components =
      static_cast<opj_image_cmptparm_t*>(malloc((original->numcomps + 2U) * sizeof(opj_image_cmptparm_t)));
  ...
  l_new_image->comps[0].factor = l_new_image->comps[1].factor = l_new_image->comps[2].factor =
      original->comps[0].factor;
  l_new_image->comps[0].alpha = l_new_image->comps[1].alpha = l_new_image->comps[2].alpha = original->comps[0].alpha;
  l_new_image->comps[0].resno_decoded = l_new_image->comps[1].resno_decoded = l_new_image->comps[2].resno_decoded =
