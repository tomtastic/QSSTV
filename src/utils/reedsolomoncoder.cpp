...
bool reedSolomonCoder::decode(QByteArray& ba, QString fn, QString& newFileName, QByteArray& baFile, QString extension,
  ...
  if (extension == "rs1")
    fileType = RST1;
  else if (extension == "rs2")
    fileType = RST2;
  else if (extension == "rs3")
    fileType = RST3;
  else if (extension == "rs4")
    fileType = RST4;
  else {
    return false;
  }
  ...
  distribute(reinterpret_cast<unsigned char*>(tr_buf.data()), reinterpret_cast<unsigned char*>(ec_buf.data()), bep_size,
             rs_bsize, DECODE);
  if (!decode_and_write()) {
    //      fpout.close();
    return false;
...
bool reedSolomonCoder::encode(QByteArray& ba, QString extension, eRSType rsType) {
  ...
  for (i = 1; i < bep_size; i++) {
    ...
    ec_buf.resize(ec_buf.size() + RSBSIZE - rs_dsize);
    rse32((reinterpret_cast<dtype*>(ec_buf.data()) + i * rs_bsize),
          (reinterpret_cast<dtype*>(ec_buf.data()) + i * rs_bsize + rs_dsize));
  }
  ba.resize(ec_buf.size());
  distribute(reinterpret_cast<unsigned char*>(ec_buf.data()), reinterpret_cast<unsigned char*>(ba.data()), bep_size,
             rs_bsize, ENCODE);
  return true;
}
