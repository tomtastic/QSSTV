...
bool hybridCrypt::enCrypt(QByteArray* ba) {
  ...
  hstr = QChar(63) + hcFtpRemoteHost + QChar(34) + hcFtpLogin + QChar(60) + hcFtpPassword + QChar(62) +
         hcFtpRemoteDirectory + QChar(58);
...
bool hybridCrypt::getParam(const QString& resultStr) {
  ...
  addToLog(QString("host: %1, login: %2, pwd: %3, dir: %4")
               .arg(hcFtpRemoteHost)
               .arg(hcFtpLogin)
               .arg(hcFtpPassword)
               .arg(hcFtpRemoteDirectory),
           LOGALL);
  return true;
}
