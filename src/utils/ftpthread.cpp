...
QString commandStr[15] = {"None", "SetTxMode", "SetProxy", "ConnectToHost", "Login", "Close",  "List",      "Cd",
...
void ftpThread::doConnect() {
  ...
  if (isUnconnected() && !connectPending) {
    ...
    //      addToLog(QString("'%1' already connected to %2,
    //      %3").arg(idName).arg(host).arg(commandStr[qftpPtr->currentCommand()]),LOGFTPTHREAD);
  }
}
...
void ftpThread::uploadFile(QString source, QString destination) {
  ...
  addToLog(QString("'%1' put '%2', %3 bytes. id=%4").arg(idName).arg(destination).arg(sourceFn->size()).arg(id),
           LOGFTPTHREAD);
}
...
void ftpThread::rename(QString source, QString destination) {
  ...
  addToLog(QString("Name=%1 source=%2 destination=%3 id:%4").arg(idName).arg(source).arg(destination).arg(id),
           LOGFTPTHREAD);
}
...
void ftpThread::slotCommandFinished(int id, bool err) {
  ...
  addToLog(QString("CommandFinished '%1' id:%2,%3 error:%4")
               .arg(idName)
               .arg(id)
               .arg(commandStr[qftpPtr->currentCommand()])
               .arg(err),
           LOGFTPTHREAD);
  switch (cmd) {
    case QFtp::Login:
