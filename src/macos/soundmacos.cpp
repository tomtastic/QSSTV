...
static AudioObjectPropertyAddress DevicesPropertyAddress = {.mSelector = kAudioHardwarePropertyDevices,
...
static std::vector<AudioObjectID> getAudioDevices() {
  ...
  ret =
      AudioObjectGetPropertyData(kAudioObjectSystemObject, &DevicesPropertyAddress, 0, nullptr, &Size, Devices.data());
  if (ret) {
...
std::string getDeviceName(AudioObjectID Id) {
  ...
  OSStatus ret = AudioObjectGetPropertyData(Id, &DeviceNamePropertyAddress, 0, nullptr, &Size, &DeviceName);
  if (ret) {
...
std::string getDeviceUID(AudioObjectID Id) {
  ...
  CFString DeviceName;
  UInt32 Size = sizeof(DeviceName);
  OSStatus ret = AudioObjectGetPropertyData(Id, &DeviceUIDPropertyAddress, 0, nullptr, &Size, &DeviceName);
  if (ret) {
