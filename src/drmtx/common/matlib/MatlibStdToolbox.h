...
/* Helpfunctions **************************************************************/
inline CReal Min(const CReal& rA, const CReal& rB) { return rA < rB ? rA : rB; }
...
void Min(CReal& rMinVal /* out */, int& iMinInd /* out */, const CMatlibVector<CReal>& rvI /* in */);

inline CReal Min(const CReal& r1, const CReal& r2, const CReal& r3, const CReal& r4) {
  return Min(Min(Min(r1, r2), r3), r4);
}
inline CReal Min(const CReal& r1, const CReal& r2, const CReal& r3, const CReal& r4, const CReal& r5, const CReal& r6,
                 const CReal& r7, const CReal& r8) {
  return Min(Min(Min(Min(Min(Min(Min(r1, r2), r3), r4), r5), r6), r7), r8);
}


inline CReal Max(const CReal& rA, const CReal& rB) { return rA > rB ? rA : rB; }
...
void Max(CReal& rMaxVal /* out */, int& iMaxInd /* out */, const CMatlibVector<CReal>& rvI /* in */);

inline CReal Max(const CReal& r1, const CReal& r2, const CReal& r3) { return Max(Max(r1, r2), r3); }
inline CReal Max(const CReal& r1, const CReal& r2, const CReal& r3, const CReal& r4, const CReal& r5, const CReal& r6,
                 const CReal& r7) {
  return Max(Max(Max(Max(Max(Max(r1, r2), r3), r4), r5), r6), r7);
}


inline CMatlibVector<CReal> Ones(const int iLen) { _VECOP(CReal, iLen, (CReal)1.0); }
inline CMatlibVector<CReal> Zeros(const int iLen) { _VECOP(CReal, iLen, (CReal)0.0); }


inline CReal Real(const CComplex& cI) { return cI.real(); }
inline CMatlibVector<CReal> Real(const CMatlibVector<CComplex>& cvI) { _VECOP(CReal, cvI.GetSize(), Real(cvI[i])); }

inline CReal Imag(const CComplex& cI) { return cI.imag(); }
inline CMatlibVector<CReal> Imag(const CMatlibVector<CComplex>& cvI) { _VECOP(CReal, cvI.GetSize(), Imag(cvI[i])); }
...
inline CMatlibVector<CReal> Abs(const CMatlibVector<CReal>& fvI) { _VECOP(CReal, fvI.GetSize(), Abs(fvI[i])); }

inline CReal Abs(const CComplex& cI) { return abs(cI); }
inline CMatlibVector<CReal> Abs(const CMatlibVector<CComplex>& cvI) { _VECOP(CReal, cvI.GetSize(), Abs(cvI[i])); }

inline CReal Angle(const CComplex& cI) { return arg(cI); }
inline CMatlibVector<CReal> Angle(const CMatlibVector<CComplex>& cvI) { _VECOP(CReal, cvI.GetSize(), Angle(cvI[i])); }


/* Trigonometric functions */
inline CReal Sin(const CReal& fI) { return sin(fI); }
template <class T>
inline CMatlibVector<T> Sin(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), sin(vecI[i]));
}

inline CReal Cos(const CReal& fI) { return cos(fI); }
template <class T>
inline CMatlibVector<T> Cos(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), cos(vecI[i]));
}

inline CReal Tan(const CReal& fI) { return tan(fI); }
template <class T>
inline CMatlibVector<T> Tan(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), tan(vecI[i]));
}

inline CReal Sinh(const CReal& fI) { return sinh(fI); }
template <class T>
inline CMatlibVector<T> Sinh(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), sinh(vecI[i]));
}

inline CReal Cosh(const CReal& fI) { return cosh(fI); }
template <class T>
inline CMatlibVector<T> Cosh(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), cosh(vecI[i]));
}

inline CReal Tanh(const CReal& fI) { return tanh(fI); }
template <class T>
inline CMatlibVector<T> Tanh(const CMatlibVector<T>& vecI) {
...
template <class T>
inline CMatlibVector<T> Sqrt(const CMatlibVector<T>& vecI) {
...
template <class T>
inline CMatlibVector<T> Exp(const CMatlibVector<T>& vecI) {
...
template <class T>
inline CMatlibVector<T> Log(const CMatlibVector<T>& vecI) {
  _VECOP(T, vecI.GetSize(), log(vecI[i]));
}

inline CReal Log10(const CReal& fI) { return log10(fI); }
template <class T>
inline CMatlibVector<T> Log10(const CMatlibVector<T>& vecI) {
...
template <class T>
inline T Mean(const CMatlibVector<T>& vecI) {
  return Sum(vecI) / vecI.GetSize();
}
template <class T>
inline T Std(CMatlibVector<T>& vecI) {
...
inline CMatlibVector<CReal> Fix(const CMatlibVector<CReal>& fvI) { _VECOP(CReal, fvI.GetSize(), Fix(fvI[i])); }

inline CReal Floor(const CReal& fI) { return floor(fI); }
inline CMatlibVector<CReal> Floor(const CMatlibVector<CReal>& fvI) { _VECOP(CReal, fvI.GetSize(), Floor(fvI[i])); }

inline CReal Ceil(const CReal& fI) { return ceil(fI); }
inline CMatlibVector<CReal> Ceil(const CMatlibVector<CReal>& fvI) { _VECOP(CReal, fvI.GetSize(), Ceil(fvI[i])); }

inline CReal Round(const CReal& fI) { return Floor(fI + static_cast<CReal>(0.5)); }
inline CMatlibVector<CReal> Round(const CMatlibVector<CReal>& fvI) { _VECOP(CReal, fvI.GetSize(), Round(fvI[i])); }

inline CReal Sign(const CReal& rI) { return rI == 0 ? 0 : rI > 0 ? 1 : -1; }

inline int Mod(const int ix, const int iy) { return ix < 0 ? (ix % iy + iy) % iy : ix % iy; }
...
inline CMatlibMatrix<CComplex> TranspH(const CMatlibMatrix<CComplex>& cmI) {
  return Conj(Transp(cmI));
} /* With conjugate complex */
...
CMatlibVector<CReal> FftFilt(const CMatlibVector<CComplex>& rvH, const CMatlibVector<CReal>& rvI,
                             CMatlibVector<CReal>& rvZ, const CFftPlans& FftPlans = CFftPlans());


/* Numerical integration */
typedef CComplex(MATLIB_CALLBACK_QAUD)(CReal rX); /* Callback function definition */
CComplex Quad(MATLIB_CALLBACK_QAUD f, const CReal a, const CReal b, const CReal errorBound = 1.e-6);


/* Implementation **************************************************************
   (the implementation of template classes must be in the header file!) */
template <class T>
inline T Sum(const CMatlibVector<T>& vecI) {
  const int iSize = vecI.GetSize();
  T SumRet = 0;
  for (int i = 0; i < iSize; i++) SumRet += vecI[i];

  return SumRet;
}

template <class T>
inline T Var(const CMatlibVector<T>& vecI) {
  const int iSize = vecI.GetSize();
  ...
  T tRet = 0;
  for (int i = 0; i < iSize; i++) tRet += (vecI[i] - tMean) * (vecI[i] - tMean);
