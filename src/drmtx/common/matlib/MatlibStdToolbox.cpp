    ...
    do {
      ...
      if ((Abs(error) <= bound) || (Abs(v1 + v2) < Abs(value) * ru)) {
        ...
        if (m == j) {
          ...
          value += _integral(f, left, x[j] + 2 * h, bound, integralBound, integralError, ru);
        } else {
          /* No, we are not */
          left = x[j];
...
CMatlibVector<CComplex> Ifft(const CMatlibVector<CComplex>& cvI, const CFftPlans& FftPlans) {
  ...
  for (i = 0; i < n; i++) {
    cvReturn[i] = CComplex(pFftwComplexOut[i][0] * scale, pFftwComplexOut[i][1] * scale);
...
CMatlibVector<CComplex> rfft(const CMatlibVector<CReal>& fvI, const CFftPlans& FftPlans) {
  ...
  CMatlibVector<CComplex> cvReturn(iShortLength
                                       /* Include Nyquist frequency in case of even N */
                                       + 1,
                                   VTY_TEMP);
