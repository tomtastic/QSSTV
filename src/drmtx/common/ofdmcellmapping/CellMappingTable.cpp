...
void CCellMappingTable::MakeTable(ERobMode eNewRobustnessMode, ESpecOcc eNewSpectOccup) {
  ...
  iIndexDCFreq = static_cast<int>(static_cast<_REAL>(VIRTUAL_INTERMED_FREQ) * iFFTSizeN / SOUNDCRD_SAMPLE_RATE);
  ...
  for (iSym = 0; iSym < iNumSymbolsPerSuperframe; iSym++) {
    ...
    for (iCar = iCarrierKmin; iCar < iCarrierKmax + 1; iCar++) {
     ...
     position */
        ...
        if (bIsBoostedPilot) {
          matcPilotCells[iSym][iCarArrInd] = Polar2Cart(2, iScatPilPhase);

          /* Add flag for boosted pilot */
          matiMapTab[iSym][iCarArrInd] |= CM_BOOSTED_PI;
        } else {
          matcPilotCells[iSym][iCarArrInd] = Polar2Cart(sqrt(static_cast<_REAL>(2.0)), iScatPilPhase);
        }
      }
     ...
     and second pilot, not No. 28 (NUM_FREQ_PILOTS - 1) */
        ...
        if (bIsFreqPilSpeciCase)
          matcPilotCells[iSym][iCarArrInd] =
              Polar2Cart(sqrt(static_cast<_REAL>(2.0)), mod(piTableFreqPilots[iFreqPilotsCounter * 2 + 1] + 512, 1024));
        else
          matcPilotCells[iSym][iCarArrInd] =
  ...
  clear all;close all;load PilotCells.dat;subplot(211),mesh(abs(complex(PilotCells(:,1:2:end),
PilotCells(:,2:2:end))));subplot(212),mesh(angle(complex(PilotCells(:,1:2:end), PilotCells(:,2:2:end))))
...
*/
  ...
  for (int z = 0; z < iNumSymbolsPerSuperframe; z++) {
    for (int v = 0; v < iNumCarrier; v++)
      fprintf(pFile, "%e %e ", matcPilotCells[z][v].real(), matcPilotCells[z][v].imag());
