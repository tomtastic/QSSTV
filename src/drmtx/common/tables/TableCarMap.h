...
inline CComplex MinDist16QAM(const CComplex cI) {
  ...
  return CComplex(
      /* Real axis minimum distance */
      Min(Abs(rTableQAM16[0][0] - Real(cI)), Abs(rTableQAM16[1][0] - Real(cI)), Abs(rTableQAM16[2][0] - Real(cI)),
          Abs(rTableQAM16[3][0] - Real(cI))),
      /* Imaginary axis minimum distance */
      Min(Abs(rTableQAM16[0][1] - Imag(cI)), Abs(rTableQAM16[1][1] - Imag(cI)), Abs(rTableQAM16[2][1] - Imag(cI)),
...
inline CComplex MinDist64QAM(const CComplex cI) {
  ...
  return CComplex(
      /* Real axis minimum distance */
      Min(Abs(rTableQAM64SM[0][0] - Real(cI)), Abs(rTableQAM64SM[1][0] - Real(cI)), Abs(rTableQAM64SM[2][0] - Real(cI)),
          Abs(rTableQAM64SM[3][0] - Real(cI)), Abs(rTableQAM64SM[4][0] - Real(cI)), Abs(rTableQAM64SM[5][0] - Real(cI)),
          Abs(rTableQAM64SM[6][0] - Real(cI)), Abs(rTableQAM64SM[7][0] - Real(cI))),
      /* Imaginary axis minimum distance */
      Min(Abs(rTableQAM64SM[0][1] - Imag(cI)), Abs(rTableQAM64SM[1][1] - Imag(cI)), Abs(rTableQAM64SM[2][1] - Imag(cI)),
          Abs(rTableQAM64SM[3][1] - Imag(cI)), Abs(rTableQAM64SM[4][1] - Imag(cI)), Abs(rTableQAM64SM[5][1] - Imag(cI)),
...
const int iTableTimePilRobModA[RMA_NUM_TIME_PIL][2] = {
    {6, 973},  {7, 205}, {11, 717}, {12, 264}, {15, 357}, {16, 357}, {23, 952}, {29, 440},
...
const int iTableTimePilRobModB[RMB_NUM_TIME_PIL][2] = {
    {6, 304},  {10, 331}, {11, 108}, {14, 620}, {17, 192}, {18, 704}, {27, 44},  {28, 432},
...
const int iTableTimePilRobModE[RME_NUM_TIME_PIL][2] = {
    {7, 432}, {8, 331}, {13, 108}, {14, 620}, {21, 192}, {22, 704}, {26, 44}, {27, 304},
};
...
const int iScatPilWRobModA[SIZE_ROW_WZ_ROB_MOD_A][SIZE_COL_WZ_ROB_MOD_A] = {
    {228, 341, 455}, {455, 569, 683}, {683, 796, 910}, {910, 0, 114}, {114, 228, 341}};
const int iScatPilZRobModA[SIZE_ROW_WZ_ROB_MOD_A][SIZE_COL_WZ_ROB_MOD_A] = {
    {0, 81, 248}, {18, 106, 106}, {122, 116, 31}, {129, 129, 39}, {33, 32, 111}};
...
const int iScatPilZRobModE[SIZE_ROW_WZ_ROB_MOD_E][SIZE_COL_WZ_ROB_MOD_E] = {
    {0, 57, 164, 64, 12}, {168, 255, 161, 106, 118}, {25, 232, 132, 233, 38}, {168, 255, 161, 106, 118}};
const int iScatPilQRobModE = 10;

/* Gain definitions of scattered pilots ------------------------------------- */
#define NUM_BOOSTED_SCAT_PILOTS 4
const int iScatPilGainRobModA[2][NUM_BOOSTED_SCAT_PILOTS] = {{2, 4, 50, 54}, {2, 6, 54, 58}};

const int iScatPilGainRobModB[2][NUM_BOOSTED_SCAT_PILOTS] = {{1, 3, 43, 45}, {1, 3, 49, 51}};
