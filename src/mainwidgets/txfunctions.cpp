...
const QString txStateStr[txFunctions::TXTEST + 1] = {
    "IDLE",       "ACTIVE",        "SENDTONE",    "SENDWFID",      "SENDCWID",         "SENDDRM",
    "SENDDRMPIC", "SENDDRMBINARY", "SENDDRMBSR",  "SENDDRMFIX",    "SENDDRMTXT",       "SSTVIMAGE",
    "SSTVPOST",   "RESTART",       "PREPARESSTV", "PREPAREDRMPIC", "PREPAREDRMBINARY", "TXTEST"};
...
void txFunctions::setDRMTxParams(drmTxParams params) { drmTxPtr->setTxParams(params); }
...
void txFunctions::startTX(etxState state) { switchTxState(state); }
...
void txFunctions::sendTestPattern() { syncBurst(); }
...
void txFunctions::forgetTxFileName() { drmTxPtr->forgetTxFileName(); }
...
void txFunctions::txTestPattern(imageViewer* ivPtr, etpSelect sel) { sstvTxPtr->createTestPattern(ivPtr, sel); }
