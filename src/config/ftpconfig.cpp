...
void ftpConfig::getParams() {
  ...
  changed = false;
  if (enableFTPCopy != enableFTP || ftpPortCopy != ftpPort || ftpRemoteHostCopy != ftpRemoteHost ||
      ftpRemoteSSTVDirectoryCopy != ftpRemoteSSTVDirectory || ftpRemoteDRMDirectoryCopy != ftpRemoteDRMDirectory ||
      ftpLoginCopy != ftpLogin || ftpPasswordCopy != ftpPassword ||
      ftpDefaultImageFormatCopy != ftpDefaultImageFormat || ftpSaveFormatCopy != ftpSaveFormat)
    changed = true;
}
