...
repeaterConfig::repeaterConfig(QWidget* parent) : baseConfig(parent), ui(new Ui::repeaterConfig) {
  ...
  connect(ui->repeaterIdleTemplateBrowseButton, &QPushButton::clicked, this,
          &repeaterConfig::slotRepeaterIdleTemplateBrowse);
...
void repeaterConfig::getParams() {
  ...
  changed = false;
  if (repeaterEnabledCopy != repeaterEnabled || repeaterImageIntervalCopy != repeaterImageInterval ||
      repeaterTxModeCopy != repeaterTxMode || repeaterIdleTxModeCopy != repeaterIdleTxMode ||
      repeaterImage1Copy != repeaterImage1 || repeaterImage2Copy != repeaterImage2 ||
      repeaterImage3Copy != repeaterImage3 || repeaterImage4Copy != repeaterImage4 ||
      repeaterAcknowledgeCopy != repeaterAcknowledge || repeaterTemplateCopy != repeaterTemplate ||
      repeaterTxDelayCopy != repeaterTxDelay || repeaterImageSizeCopy != repeaterImageSize ||
