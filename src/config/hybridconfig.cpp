...
void hybridConfig::getParams()
{
  ...
  if (enableHybridRxCopy != enableHybridRx
      ...
      || hybridFtpPortCopy != hybridFtpPort || hybridFtpRemoteHostCopy != hybridFtpRemoteHost ||
      hybridFtpRemoteDirectoryCopy != hybridFtpRemoteDirectory || hybridFtpLoginCopy != hybridFtpLogin ||
      hybridFtpPasswordCopy != hybridFtpPassword || enableHybridNotifyCopy != enableHybridNotify ||
      hybridNotifyDirCopy != hybridNotifyDir || onlineStatusDirCopy != onlineStatusDir)
    changed = true;
}
...
void hybridConfig::slotTestHybridPushButton()
{
  ...
  getParams();
  ff.test("Hybrid Ftp Test", hybridFtpRemoteHost, hybridFtpPort, hybridFtpLogin, hybridFtpPassword,
          hybridFtpRemoteDirectory + "/" + hybridFtpHybridFilesDirectory, true);
  ff.getLastErrorStr(resultStr1);

  ff.test("Notify Ftp Test", hybridFtpRemoteHost, hybridFtpPort, hybridFtpLogin, hybridFtpPassword,
          hybridFtpRemoteDirectory + "/" + hybridNotifyDir, false);
  ff.getLastErrorStr(resultStr2);
  ff.test("OnlineStus Ftp Test", hybridFtpRemoteHost, hybridFtpPort, hybridFtpLogin, hybridFtpPassword,
          hybridFtpRemoteDirectory + "/" + onlineStatusDir, false);
  ff.getLastErrorStr(resultStr3);

  QApplication::restoreOverrideCursor();
  ui->testHybridPushButton->setDisabled(false);
  QMessageBox::information(this, "FTP Site Test", QString("%1\n%2\n%3").arg(resultStr1).arg(resultStr2).arg(resultStr3),
                           QMessageBox::Ok);
}
