...
void rigConfig::getParams() {
  ...
  *cpCopy = *cp;
  ...
  changed = false;
  if (cpCopy->serialPort != cp->serialPort || cpCopy->radioModel != cp->radioModel ||
      cpCopy->civAddress != cp->civAddress || cpCopy->baudrate != cp->baudrate || cpCopy->stopbits != cp->stopbits ||
      cpCopy->databits != cp->databits || cpCopy->handshake != cp->handshake || cpCopy->enableCAT != cp->enableCAT ||
      cpCopy->enableSerialPTT != cp->enableSerialPTT || cpCopy->pttSerialPort != cp->pttSerialPort ||
      cpCopy->activeRTS != cp->activeRTS || cpCopy->activeDTR != cp->activeDTR ||
      cpCopy->nactiveRTS != cp->nactiveRTS || cpCopy->nactiveDTR != cp->nactiveDTR || cpCopy->pttType != cp->pttType ||
      cpCopy->enableXMLRPC != cp->enableXMLRPC || cpCopy->XMLRPCPort != cp->XMLRPCPort ||
      cpCopy->enableHamlibNetworkControl != cp->enableHamlibNetworkControl || cpCopy->hamlibHost != cp->hamlibHost ||
