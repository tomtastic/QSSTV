 ...
 * \class calibration
 *
 * Check first if ntp is running and it is synchronised. A dialog window will appear and show the progress of the RX and
 * TX clocks. About 10000 blocks of data will be read/written to calculate the exact timing. If the OK button is
 * pressed, the clocks will be saved for later use.
 *
 **/
 ...
 *
 * Call this function to start the calibration process. The use the results, check the return status and get the value
 * of the clocks by calling getRXClock() and getTXClock().
 *
 ...
 *
 * This slot is called when the dialog is closed by pressing CANCEL or OK. It will abort the loop executed in start and
 ...
 * @brief initialize
 *
 * @param[in] isRX: If isRX is set then the receive clock will be calibrated, else the transmit clock will be
 * calibrated.
 *
 * Start is called by exec and performs the clock calibration using NTP (Network Time Protocol).
 * It starts counting when the first 100 blocks are read or when the first 100 blocks are written in order to start with
 * a stable condition.
 * @return bool returns true if calibration is successful or false when either the CANCEL button was pressed or a read
 * NTP time error has occurred.
 *
 **/
