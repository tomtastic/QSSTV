...
bool wavIO::openFileForRead(QString fname, bool ask) {
  ...
  if (!inopf.open(QIODevice::ReadOnly)) {
    stmb = new displayMBoxEvent("WAVIO Error",
  ...
  // check the header
  if ((!checkString(waveHeader.chunkID, "RIFF")) || (!checkString(waveHeader.format, "WAVE")) ||
      (!checkString(waveHeader.subChunk1ID, "fmt ")) || (!checkString(waveHeader.subChunk2ID, "data"))) {
  ...
  if ((waveHeader.subChunk1Size != 16) || (waveHeader.audioFormat != 1) || (waveHeader.numChannels > STEREOCHANNEL) ||
      (waveHeader.sampleRate != samplingrate)
      //      ||(waveHeader.byteRate!=sizeof(SOUNDFRAME)*samplingrate)
      || (waveHeader.blockAlign != waveHeader.numChannels * 2) || (waveHeader.bitsPerSample != 16)) {
...
/**
  ...
  \return returns the number of samples read. -1 is returned if there is an error, 0 is returned on end of the file else
...
bool wavIO::writeHeader() {
  ...
  addToLog(QString("wavio write header %1 %2 %3 %4")
               .arg(waveHeader.chunkID[0])
               .arg(waveHeader.chunkID[1])
               .arg(waveHeader.chunkID[2])
               .arg(waveHeader.chunkID[3]),
           LOGWAVIO);
