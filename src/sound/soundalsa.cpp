...
bool soundAlsa::setupSoundParams(bool isCapture)
{
  ...
  if (isCapture) {
    ...
    if (!alsaErrorHandler(err, QString("Unable to set buffersize %1 for playback").arg(playbackBufferSize)))
      return false;
  }
...
void getCardList(QStringList& alsaInputList, QStringList& alsaOutputList)
{
  ...
  while (*n != nullptr) {
    ...
    if (!deviceName.contains("surround", Qt::CaseInsensitive) && !deviceName.contains("dmix", Qt::CaseInsensitive) &&
        !deviceName.contains("front", Qt::CaseInsensitive) && !deviceName.contains("plughw", Qt::CaseInsensitive) &&
        !deviceName.contains("null", Qt::CaseInsensitive) && !deviceName.contains("hdmi", Qt::CaseInsensitive) &&
        !deviceName.contains("sysdefault", Qt::CaseInsensitive) && !deviceName.contains("dsnoop", Qt::CaseInsensitive)
  ...
  if (snd_config_search(snd_config, "pcm", &pcmc) == 0) {
    ...
    snd_config_for_each(i, next, pcmc)
    {
      ...
      if (snd_config_get_id(n, &id) == 0) {
        ...
        if (deviceName == "hw" || deviceName == "plughw" || deviceName == "plug" || deviceName == "dsnoop" ||
            deviceName == "tee" || deviceName == "file" || deviceName == "null" || deviceName == "shm" ||
            deviceName == "cards" || deviceName == "rate_convert")
          continue;
