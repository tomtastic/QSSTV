...
const QString captureStateStr[soundBase::CPEND + 1] = {
    "Capture Init",           "Capture Starting",  "Capture Running", "Capture Calibrate Starting",
    "Capture Calibrate Wait", "Capture Calibrate", "Capture End"};


const QString playbackStateStr[soundBase::PBEND + 1] = {"Playback Init",        "Playback Starting",
                                                        "Playback Running",     "Playback Calibrate 1",
                                                        "Playback Calibrate 2", "Playback End"};
...
soundBase::~soundBase() { delete downsampleFilterPtr; }
...
int soundBase::captureCalibration(bool leadIn) {
  ...
  if (leadIn) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts);
    ucalibrationTime =
        static_cast<double>(ts.tv_sec) + static_cast<double>(ts.tv_nsec) / 1000000000.0 - ustartcalibrationTime;
    mutex.unlock();
    // logFilePtr->addToAux(QString("%1\t%2\t%3").arg(countAvailable).arg(calibrationFrames).arg(calibrationTime) );
  }
  addToLog(QString("read report count:%1 available %2 elapsed qtime %3, time: %4")
               .arg(count)
               .arg(countAvailable)
               .arg(calibrationTime)
               .arg(ucalibrationTime),
           LOGSOUND);
  return count;
}
...
int soundBase::play() {
  ...
  addToLog(QString("frames to write: %1 at %2 buffered:%3")
               .arg(numFrames)
               .arg(txBuffer.getReadIndex())
               .arg(txBuffer.count()),
           LOGSOUND);
...
void soundBase::switchCaptureState(ecaptureState cs) {
  addToLog(QString("Switching from captureState %1 to %2").arg(captureStateStr[captureState]).arg(captureStateStr[cs]),
           LOGSOUND);
  captureState = cs;
}
...
void soundBase::switchPlaybackState(eplaybackState ps) {
  addToLog(
      QString("Switching from playbackState %1 to %2").arg(playbackStateStr[playbackState]).arg(playbackStateStr[ps]),
      LOGSOUND);
  playbackState = ps;
}
...
void soundBase::applyAGC(qint16* buffer, int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    peakLevel = (level > peakLevel) ? level  // Immediate rise for strong signals
