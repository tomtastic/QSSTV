...
int getofdm(/*@null@ */ float* rs, float time_offset_fractional_init, float freq_offset_init,
...

  {
    ...
    p = fftwf_plan_dft_1d(Tu, reinterpret_cast<fftwf_complex*>(s), reinterpret_cast<fftwf_complex*>(S), FFTW_FORWARD,
                          FFTW_ESTIMATE);
...

  {
...

    {
      ...
      //          logfile->addToAux(QString("dt %1 %2 %3 %4
   ...
   delta_time_offset_integer, phi_freq_correction_last); */
...

    {
      ...
      /* filter epsilon_ML */
      freq_offset_ctrl = fmodf(epsilon_ML - freq_offset + PI + 100.0 * PI, 2 * PI) - PI;
      dfreq_offset_I += kI_freq_controller * freq_offset_ctrl;
      dfreq_offset_P = kP_large_freq_controller * freq_offset_ctrl +
                       threshold_freq_small_large * (kP_small_freq_controller - kP_large_freq_controller) *
