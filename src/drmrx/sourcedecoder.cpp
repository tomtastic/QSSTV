...
bool sourceDecoder::setupDataPacket(QByteArray ba)
{
  ...
  if (header & 0x40) {
    ...
    crc16_bytewise(&checksum, reinterpret_cast<unsigned char*>(currentDataPacket.ba.data()),
                   currentDataPacket.ba.size());
    if (fabs(checksum) <= DBL_EPSILON) {
  ...
  if (currentDataPacket.sessionFlag) {
    currentDataPacket.segmentNumber = ((static_cast<unsigned char>(currentDataPacket.ba.at(0)) & 0x7F)) * 256 +
                                      (static_cast<unsigned char>(currentDataPacket.ba.at(1)));
  ...
  if (currentDataPacket.userFlag) {
    ...
    if ((currentDataPacket.userAccessField & 0x10) && (lengthIndicator >= 2))
      currentDataPacket.transportID = ((static_cast<unsigned char>(currentDataPacket.ba.at(0)))) * 256 +
                                      (static_cast<unsigned char>(currentDataPacket.ba.at(1)));
    currentDataPacket.advance(lengthIndicator);
  }
  currentDataPacket.segmentSize = ((static_cast<unsigned char>(currentDataPacket.ba.at(0)) & 0x1F)) * 256 +
                                  (static_cast<unsigned char>(currentDataPacket.ba.at(1)));
  currentDataPacket.advance(2);
...
bool sourceDecoder::addHeaderSegment()
{
  ...
  tbPtr->bodySize = ((static_cast<unsigned int>(dataPtr[0])) << 20) + ((static_cast<unsigned int>(dataPtr[1])) << 12) +
                    ((static_cast<unsigned int>(dataPtr[2])) << 4) +
                    (((static_cast<unsigned int>(dataPtr[3])) & 0xF0) >> 4);

  tbPtr->headerSize = ((static_cast<unsigned int>(dataPtr[3]) & 0x0F) << 9) +
...
void sourceDecoder::addDataSegment()
{
  ...
  tbPtr->dataSegmentPtrList.at(currentDataPacket.segmentNumber)
      ->setData(currentDataPacket.ba, currentDataPacket.segmentNumber, true);
  ...
  if ((tbPtr->fileName.left(3) == ".de") || (tbPtr->fileName.left(3) == "de_")) {
    ...
    if ((enableHybridRx) && (soundRoutingInput != soundBase::SNDINFROMFILE)) {
      ...
      if (hc.deCrypt(&hybridBa)) {
        if (tbPtr->retrieveTries == 0) {
          ff->setupFtp("GetHybridImage", hc.host(), hc.port(), hc.user(), hc.passwd(),
                       hc.dir() + "/" + hybridFtpHybridFilesDirectory);
