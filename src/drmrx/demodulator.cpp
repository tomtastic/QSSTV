...
demodulator::demodulator() {
  ...
  p1 = fftwf_plan_dft_1d(256, reinterpret_cast<fftwf_complex*>(ss), reinterpret_cast<fftwf_complex*>(S), FFTW_FORWARD,
                         FFTW_ESTIMATE);
  addToLog("fftwf_plan_dft_1d stop", LOGFFT);
}

demodulator::~demodulator() { fftwf_destroy_plan(p1); }
...
void demodulator::init() {
  ...
  for (i = 0; i < 4; i++)  // number of robustness modes
  {
    ...
    {
      no_of_used_cells_per_frame_list[k++] =
          (K_min_K_max_list[1][j + (i * 6)] - K_min_K_max_list[0][j + (i * 6)] + 1 - no_of_unused_carriers_list[i]) *
          symbols_per_frame_list[i];
    }
  }
...
bool demodulator::timeSync() {
  ...
  if (!timeSyncFlag) {
    ...
    if (robustness_mode != 99) {
      addToLog(QString("numSamples %1, robustmode %2,timeoffset %3,smplrateOffsetEst %4,freqOffsetFracInit "
                       "%5,timeOffsetInteger %6")
                   .arg(numberOfSamples)
                   .arg(robustness_mode)
                   .arg(time_offset)
                   .arg(samplerate_offset_estimation)
                   .arg(frequency_offset_fractional_init)
                   .arg(time_offset_integer),
               LOGDRMDEMOD);
    }
...
bool demodulator::frequencySync() {
  ...
  if (!frequencySyncFlag) {
    ...
    for (i = 0; i < N_symbols_frequency_pilot_search; i++) {
      delta_time_offset_integer = getofdm(&rs_buffer[2 * t_smp], time_offset_fractional_init, freq_offset_init,
                                          delta_time_offset_I_init, Ts, Tu, Zi, symbol_temp, 0, 1, 1);
   ...
   */
...

    {
...

      {
        ...
        {
          tmp1 += ((S_buffer[K_dc_plus2_indx][i]).re * (S_buffer[K_dc_plus2_indx][i]).re) +
                  ((S_buffer[K_dc_plus2_indx][i]).im * (S_buffer[K_dc_plus2_indx][i]).im);
          tmp2 += ((S_buffer[K_dc_indx][i]).re * (S_buffer[K_dc_indx][i]).re) +
                  ((S_buffer[K_dc_indx][i]).im * (S_buffer[K_dc_indx][i]).im);
          tmp3 += ((S_buffer[K_max_indx][i]).re * (S_buffer[K_max_indx][i]).re) +
                  ((S_buffer[K_max_indx][i]).im * (S_buffer[K_max_indx][i]).im);
          tmp4 += ((S_buffer[K_max_plus1_indx][i]).re * (S_buffer[K_max_plus1_indx][i]).re) +
                  ((S_buffer[K_max_plus1_indx][i]).im * (S_buffer[K_max_plus1_indx][i]).im);
          tmp5 += ((S_buffer[K_min_indx][i]).re * (S_buffer[K_min_indx][i]).re) +
                  ((S_buffer[K_min_indx][i]).im * (S_buffer[K_min_indx][i]).im);
          tmp6 += ((S_buffer[K_min_minus4_indx][i]).re * (S_buffer[K_min_minus4_indx][i]).re) +
                  ((S_buffer[K_min_minus4_indx][i]).im * (S_buffer[K_min_minus4_indx][i]).im);
        }
...
bool demodulator::frameSync() {
  ...
  if (!frameSyncFlag) {
    ...
    symbol0 =
        getsymbolidx(symbol_buffer, symbols_per_frame, time_ref_cells_k, time_ref_cells_theta_1024, K_dc, K_modulo, 21);
...
bool demodulator::channelEstimation() {
  ...
  if (mode_and_occupancy_code != mode_and_occupancy_code_last) {
    ...
    K_min = K_min_K_max_list[0][spectrum_occupancy_estimation + (robustness_mode * 6)];
    K_max = K_min_K_max_list[1][spectrum_occupancy_estimation + (robustness_mode * 6)];
    carrier_per_symbol = K_max - K_min + 1;
    for (i = 0; i < 4; i++) {
    ...
    mean_energy_of_used_cells =
        static_cast<float>(no_of_used_cells_per_frame_list[spectrum_occupancy_estimation + (robustness_mode * 6)] + 3 +
                           cnt_time_ref_cells);
    rndcnt = 0;
    for (s = 0; s < symbols_per_frame; s++) {
      ...
      {
        k = k0 + (x * nnn) + (x * y * p);
        theta_1024 = ((4 * Z_256_list[robustness_mode][nnn][m]) + (p * W_1024_list[robustness_mode][nnn][m]) +
                      (p * p * (1 + s) * Q_1024)) %
                     1024;
        a = sqrtf(2.0);
        // power boost
    ...
    for (nnn = 0; nnn < y; nnn++) {
      for (i = 0; i < rndcnt; i++) {
        training_cells_k[nnn][i] = ((gain_ref_cells_k[i] - K_min + ((symbols_per_frame - nnn) * carrier_per_symbol)) %
                                    (symbols_per_frame * carrier_per_symbol)) +
                                   K_min;
      }
      ...
      for (i = 0; i < rndcnt; i++) {
...

        {
          ...
          next_pilot_cells_k[nnn][(cnt_next_pilot_cells[nnn])++] =
              ((training_cells_k[nnn][i] - K_min) % carrier_per_symbol) + K_min;
        }
      }
      ...
      for (i = 0; i < gain_ref_cells_per_window; i++) {
        PHI[i][i] += sigmaq_noise * 2.0 /
                     ((gain_ref_cells_a[gain_ref_cells_subset_index[nnn][i]]) *
                      (gain_ref_cells_a[gain_ref_cells_subset_index[nnn][i]]));
      }
  ...
  for (i = 0; i < symbols_per_frame; i++) {
    ...
    for (j = 0; j < cnt_tr_cells[ntwee]; j++) {
      gain_ref_cells_subset_nn[j] =
          ((mtwee * gain_ref_cells_per_y_symbols) + gain_ref_cells_subset_index[ntwee][j]) % gain_ref_cells_per_frame;
      training_cells_relative_index[j] =
          (gain_ref_cells_k[gain_ref_cells_subset_nn[j]] + ((symbols_per_frame - nn) * K_modulo)) %
    ...
    for (j = 0; j < cnt_tr_cells[ntwee]; j++) {
      ...
      ntc_indx = training_cells_relative_index[j] + (i * K_modulo) +
                 1; /* pa0mbo in  matlab +1 =OK trcrindx 1 lager dan in M */

      hoek = static_cast<float>(2.0 * M_PI *
                                static_cast<float>(gain_ref_cells_theta_1024[gain_ref_cells_subset_nn[j]]) / 1024.0);
      ...
      normalized_training_cells[2 * j] =
          (symbol_buffer[2 * ntc_indx] * tmpreal) - (symbol_buffer[(2 * ntc_indx) + 1] * tmpimag); /* real part */
      normalized_training_cells[(2 * j) + 1] =
          (symbol_buffer[(2 * ntc_indx) + 1] * tmpreal) + (symbol_buffer[2 * ntc_indx] * tmpimag); /* imag part */
    }
    ...
    temp2 = 0.0;
    for (j = 0; j < cnt_actual_pilots_rel_indx; j++) {
      temp1 += (actual_pilots[2 * j] * next_pilots[2 * j]) +
               (actual_pilots[(2 * j) + 1] * next_pilots[(2 * j) + 1]); /* real part */
    ...
    for (j = K_min; j <= K_max; j++) {
      ...
      tmp1 =
          (H[2 * (j - K_min)] * H[2 * (j - K_min)]) + (H[(2 * (j - K_min)) + 1] * H[(2 * (j - K_min)) + 1]) + MIN_ABS_H;
      tmp2 = H[2 * (j - K_min)] / tmp1;
      tmp3 = -H[(2 * (j - K_min)) + 1] / tmp1;
      transmission_frame_buffer[2 * trxbuf_indx] =
          (symbol_buffer[2 * symbuf_indx] * tmp2) - (symbol_buffer[(2 * symbuf_indx) + 1] * tmp3);
      transmission_frame_buffer[(2 * trxbuf_indx) + 1] =
          (symbol_buffer[2 * symbuf_indx] * tmp3) + (symbol_buffer[(2 * symbuf_indx) + 1] * tmp2);
    ...
    delta_time_offset_integer =
        getofdmsync(&rs_buffer[2 * t_smp], Ts, Tu, H, K_max - K_min + 1, delta_freq_offset, Zi, symbol_temp, 0, 1, 1);
  ...
  addToLog(QString("Rate %1 %2 %3")
               .arg(smp_rate_conv_fft_phase_diff)
