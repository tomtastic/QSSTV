...
bool demodulator::channelEstimation() {
  ...
  if (mode_and_occupancy_code != mode_and_occupancy_code_last) {
    ...
    mean_energy_of_used_cells = static_cast<float>(
        no_of_used_cells_per_frame_list[spectrum_occupancy_estimation + robustness_mode * 6] + 3 + cnt_time_ref_cells);
    rndcnt = 0;
  ...
  for (i = 0; i < symbols_per_frame; i++) {
    ...
    for (j = 0; j < cnt_tr_cells[ntwee]; j++) {
      ...
      ntc_indx =
          training_cells_relative_index[j] + i * K_modulo + 1; /* pa0mbo in  matlab +1 =OK trcrindx 1 lager dan in M */
