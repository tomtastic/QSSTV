...
void channel_decoding(void) {
  ...
  static int RatesSM64[4][3] = {{1, 5, 10}, {3, 8, 11}, {5, 10, 12}, {8, 11, 13}};
    ...
    if (enough_frames == 0) {
      for (i = 0; i < lMSC; i++) {
        if ((MSC_Demapper[frame_index - 1][i] > max_index_equal_samples) ||
            (MSC_Demapper[frame_index - 1][i] < min_index_equal_samples))
      ...
      n_SPPhard =
          msdhardmsc(received_real, received_imag, lMSC, SNR_estimation, N1, L, rowdimL, coldimL, Lvspp, Deinterleaver,
                     PL, MSD_ITER, 1, SPPhard, VSPPhard, &res_iters, &calc_variance, noise_signal);
