...
int viterbi_decode(float*, int, int, signed char*, signed char*, signed char*, char*, char*, int, int*, int, int,
                   char*);
int msdhardmsc(double* received_real, double* received_imag, int Lrxdata, double* snr, int N1, double* L, int rowdimL,
  ...
  for (level = 0; level < no_of_levels; level++) {
    no_of_bits += static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]) + 6 +
      ...
      { /* for real and imaginary part */
...

        {
          sample = static_cast<float>(
              received[sample_index >> (1 - HMmix)]); /* extract real or imaginary part respectively */
          closest_zero = fabs(sample - metric[static_cast<int>(hardpoints_ptr[sample_index])]);
          ...
          for (subset_point = (0x1 << (level + 1)); subset_point < (0x1 << no_of_levels);
          ...
          for (subset_point = (0x3 << level); subset_point < (0x1 << no_of_levels);
               subset_point += (0x1 << (level + 1)))
...
#ifdef CONSIDERING_SNR
          SNR = static_cast<float>(signal_to_noise_ratio[sample_index >> (1 - HMmix)]);
...

  {
...

    {
...

      {
        ...
        {
          diff = 0;
          for (sample_index = 0; sample_index < static_cast<int>((2 - HMmix) * N * sizeof(char) / sizeof(int));
               sample_index++) {
            diff += ((reinterpret_cast<int*>(hardpoints))[sample_index] ^
  ...
  for (level = (Lvspp != 0); level < no_of_levels; level++) {
    ...
    for (m = static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]) + 6;
         m < static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]) + 6 + static_cast<int>(L1_imag[level]);
         m++) {
      output_ptr[n++] = static_cast<double>(infoout[level][m] ^ PRBS_BIT(PRBS_reg));
      PRBS_SHIFT(PRBS_reg);
    }
  ...
  for (level = (Lvspp != 0); level < no_of_levels; level++) {
    for (m = static_cast<int>(L1_real[level]); m < static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]);
         m++) {
      output_ptr[n++] = static_cast<double>(infoout[level][m] ^ PRBS_BIT(PRBS_reg));
      PRBS_SHIFT(PRBS_reg);
    }
    for (m = static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]) + 6 + static_cast<int>(L1_imag[level]);
         m < static_cast<int>(L1_real[level]) + static_cast<int>(L2_real[level]) + 6 +
                 static_cast<int>(L1_imag[level]) + static_cast<int>(L2_imag[level]);
         m++) {
      output_ptr[n++] = static_cast<double>(infoout[level][m] ^ PRBS_BIT(PRBS_reg));
      PRBS_SHIFT(PRBS_reg);
    }
  ...
  PRBS_INIT(PRBS_reg);
  if (Lvspp != 0) {
    printf("msdhardmsc: There is a very strongly protected part, but no variable to put it into!");
  }
  no_of_bits = Lvspp;
  output_ptr = VSPPhard;
  for (m = 0; m < Lvspp; m++) {
  ...
  for (sample_index = 0; sample_index < N; sample_index++) {
    ...
    dist = (sample - metric_real[static_cast<int>(hardpoints[(2 - HMmix) * sample_index])]);
